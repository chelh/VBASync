...
namespace VbaSync {
    public class IniFile {
        ...
        public Guid? GetGuid(string subject, string key) {
            string s;
            Guid u;
            if (Dict.TryGetValue($"{subject}\0{key}", out s) && Guid.TryParse(s, out u)) {
                return u;
            }
            return null;
        }
        ...
        public int? GetInt(string subject, string key) {
            string s;
            int i;
            if (Dict.TryGetValue($"{subject}\0{key}", out s) && (int.TryParse(s, out i) || ((s?.Length ?? 0) > 2 && int.TryParse(s?.Substring(1, s.Length - 2), out i)))) {
                return i;
            }
            return null;
        }
        ...
        public string GetString(string subject, string key) {
            string s;
            if (Dict.TryGetValue($"{subject}\0{key}", out s) && s != null && s.Length >= 2 && s[0] == '"' && s[s.Length - 1] == '"') {
        ...
        public Version GetVersion(string subject, string key) {
            string s;
            Version v;
            if (Dict.TryGetValue($"{subject}\0{key}", out s) && Version.TryParse(s, out v)) {
                return v;
            }
            return null;
        }
